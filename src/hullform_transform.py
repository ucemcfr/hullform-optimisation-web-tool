...
plt.plot(range(0,len(section_areas)), section_areas)
plt.show()
...
def calc_hydrostatics(section_areas, stations, lwl):
    ...
    parallel_mid_bod_x = x_1d[y_1d == max(y_1d)]
...
##############
# Lackenby transform
def stn_shift(stn_x, dLp_f, dLp_a, dCp, Lcb, dLcb, Cp, section_areas, stations_x, lwl, parallel_mid_bod_extents):
    ###### NB if there are issues check that all the sign conventions are correct i.e. are +ve and -ve x values the same throughout
    # need to find:
    # Cp f,a
    # x bar f,a
    # length of parallel mid body f,a
    midships = lwl/2
    ...
    fore_areas = section_areas[stations_x>midships] # this will be ordered from aft most to fwd most I think
    aft_areas = section_areas[stations_x<midships] # this will be ordered from aft most to fwd most I think
    aft_stations = stations_x[stations_x<midships]
    fore_stations = stations_x[stations_x>midships]
    ...
    aft_areas.append(midship_area)
    aft_stations.append(midships)
    fore_areas.insert(0, midship_area)
    fore_stations.insert(0, midships)
    aft_stns_from_midship = [(stn-lwl/2) for stn in aft_stations]
    fore_stns_from_midship = [(stn - lwl / 2) for stn in fore_stations]

    # calculate centroid (x bar) for fore and aft parts
    # calculate area moments
    aft_area_moments = []
    aft_area_second_moments = []

    # TODO should this not be to the CENTRE of each strip not the actual section poisition? check
    for i in range(0, len(aft_stations)):
        aft_moment = aft_areas[i] * aft_stations[i]
        aft_area_moments.append(aft_moment)
        aft_second_moment = aft_areas[i] * aft_stations[i]**2 # TODO check this is correct
        aft_area_second_moments.append(aft_second_moment)

    # calculates the centroid as mm from baseline
    x_bar_a = sum(aft_area_moments) / sum(aft_areas)
    I_a = sum(aft_area_second_moments)
    S_a = sum(aft_areas) # TODO check the units of this
    k_a = I_a / S_a

    fore_area_moments = []
    fore_area_second_moments = []

    for i in range(0, len(fore_stations)):
        fore_moment = fore_areas[i] * fore_stations[i]
        fore_area_moments.append(fore_moment)
        fore_second_moment = fore_areas[i] * fore_stations[i]**2 # TODO check this is correct
        fore_area_second_moments.append(fore_second_moment)

    # calculates the centroid as mm from baseline
    x_bar_f = sum(fore_area_moments) / sum(fore_areas)
    I_f = sum(fore_area_second_moments)
    S_f = sum(fore_areas) # TODO check the units of this
    k_f = I_f / S_f
    ...
    vol_f = simps(fore_areas, fore_stations, even='avg')
    #calculating fwd cp
    Cp_f = vol_f / (max(fore_areas) * lwl/2)
    # calculating aft volume
    vol_a = simps(aft_areas, aft_stations, even='avg')
    # calculating aft cp
    Cp_a = vol_a / (max(aft_areas) * lwl/2)

    # calculate length of parallel mid body
    Lp_f = parallel_mid_bod_extents[1] - midships
    Lp_a = midships - parallel_mid_bod_extents[0] #TODO check this is giving the correct answer
    ...
    x_fa = (stn_x-(lwl/2)) / (lwl/2) # TODO is this total length or waterline length? How am I going to get lwl out of the offsets?
    ...
    k_f = I_f / S_f # TODO what the fuck are these?! look at computational geometry book
    k_a = I_a / S_a
    ...
    h_f = Cp_f * ((B_f / Cp_f) * 1 - ((dLp_f * (1 - Cp_f)) / dCp_f * (1 - Lp_f)) + (dLp_f * (1 - 2 * x_bar_f)) / (dCp_f * (1 - Lp_f)))
    h_a = Cp_a * ((B_a / Cp_a) * 1 - ((dLp_a * (1 - Cp_a)) / dCp_a * (1 - Lp_a)) + (dLp_a * (1 - 2 * x_bar_a)) / (dCp_a * (1 - Lp_a)))
...
child_z_offsets = np.concatenate((delta * parent_z_offsets[np.where(parent_z_offsets <= 0)],
                                  epsilon * parent_z_offsets[np.where(parent_z_offsets > 0)]))
